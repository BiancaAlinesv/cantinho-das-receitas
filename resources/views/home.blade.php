...
<section class="section container"><div class="section-heading"><div><span class="eyebrow">Para começar</span><h2>Escolha uma categoria</h2></div><a class="text-link" href="{{ route('receitas.listar') }}">Ver todas <span>→</span></a></div><div class="category-grid" id="categorias">@foreach ($categories as $category)<a href="{{ route('receitas.listar', ['categoria' => $category['name']]) }}" class="category-card" aria-label="Ver receitas da categoria {{ $category['name'] }}"><span class="category-icon" aria-hidden="true">{{ $category['icon'] }}</span><strong>{{ $category['name'] }}</strong><small>{{ $category['count'] }} {{ $category['count'] === 1 ? 'receita' : 'receitas' }}</small></a>@endforeach</div></section>
