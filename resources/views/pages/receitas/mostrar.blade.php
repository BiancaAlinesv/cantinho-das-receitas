...
new class extends Component {
    ...
    public function mount(Receita $receita): void
    {
        ...
        $this->receita = $receita->load(['categoria', 'ingredientes.ingrediente', 'passos', 'user', 'fonte']);
...
@php
    ...
    $receita = $receita->load(['categoria', 'ingredientes.ingrediente', 'passos', 'user', 'fonte']);
    $seoSchema = [
        '@context' => 'https://schema.org',
        '@type' => 'Recipe',
        'name' => $receita->titulo,
        'image' => $receita->foto_url ? [$receita->foto_url] : null,
        ...
        'recipeInstructions' => $receita->passos->map(fn ($step) => ['@type' => 'HowToStep', 'text' => $step->descricao])->values()->all(),
        'datePublished' => optional($receita->published_at)->toIso8601String(),
    ...
    $relacionadas = app(ReceitaRelacionadaService::class)->buscar($receita);
@endphp

<x-layouts.public :title="$receita->titulo.' — Cantinho das Receitas'" :description="\Illuminate\Support\Str::limit($receita->descricao, 155)" :image="$receita->foto_url ?: null" og-type="article">
    ...
    <article class="recipe-detail container">
        ...
        <div class="detail-hero"><img src="{{ $receita->foto_url ?: 'https://placehold.co/1200x700/f3eadf/bd5b32?text=Receita' }}" alt="{{ $receita->titulo }}" width="1200" height="700" fetchpriority="high" decoding="async"><div class="detail-hero-overlay"><span class="recipe-category">{{ $receita->categoria->nome }}</span><h1>{{ $receita->titulo }}</h1><p>{{ $receita->descricao }}</p></div></div>
        <div class="detail-layout"><div class="detail-main"><div class="recipe-facts"><div><span>Preparo</span><strong>{{ $receita->tempo_preparo_min }} min</strong></div><div><span>Cozimento</span><strong>{{ $receita->tempo_cozimento_min ?: '—' }} min</strong></div><div><span>Total</span><strong>{{ $receita->tempoTotalMin() }} min</strong></div><div><span>Rendimento</span><strong>{{ $receita->porcoes }} porções</strong></div><div><span>Dificuldade</span><strong>{{ ucfirst($receita->dificuldade) }}</strong></div></div><div class="detail-social"><livewire:interacoes.like-button :receita="$receita" :key="'like-'.$receita->id" /><livewire:interacoes.favorite-button :receita="$receita" :key="'favorite-'.$receita->id" /></div><livewire:interacoes.recipe-status-button :receita="$receita" :key="'status-'.$receita->id" /></div><livewire:receitas.portion-calculator :receita="$receita" :key="'calculator-'.$receita->id" /><section class="preparation"><div class="detail-section-heading"><div><span class="eyebrow">Passo a passo</span><h2>Modo de preparo</h2></div></div><ol>@foreach($receita->passos as $step)<li><span>{{ $loop->iteration }}</span><p>{{ $step->descricao }}</p></li>@endforeach</ol></section><livewire:interacoes.rating-stars :receita="$receita" :key="'rating-'.$receita->id" /><livewire:interacoes.share-recipe :receita="$receita" :key="'share-'.$receita->id" /><livewire:interacoes.comments-list :receita="$receita" :key="'comments-'.$receita->id" /></div><aside class="detail-aside"><div class="aside-card"><span class="eyebrow">Essa receita é</span><div class="aside-rating"><strong>★ {{ number_format((float)$receita->nota_media, 1, ',', '.') }}</strong><span>{{ $receita->total_avaliacoes }} avaliações</span></div><div class="aside-line"><span>Custo</span><strong>{{ ucfirst($receita->custo) }}</strong></div><div class="aside-line"><span>Visualizações</span><strong>{{ number_format($receita->visualizacoes_total, 0, ',', '.') }}</strong></div></div><div class="aside-note"><span>✦</span><p>Uma receita feita para ser compartilhada. Se preparar, conte para a gente como ficou.</p></div></aside></div>
        @if($receita->fonte)<section class="recipe-source-note container"><span class="eyebrow">Origem guardada</span><p>Encontrada em <strong>{{ $receita->fonte->nome_fonte ?: ucfirst($receita->fonte->tipo) }}</strong>@if($receita->fonte->url) · <a href="{{ $receita->fonte->url }}" target="_blank" rel="noopener noreferrer">Ver receita original ↗</a>@endif</p></section>@endif<section class="related-section"><div class="section-heading"><div><span class="eyebrow">Mais ideias para você</span><h2>Você também pode gostar</h2></div><a class="text-link" href="{{ route('receitas.listar') }}">Ver todas <span>→</span></a></div><div class="recipe-grid related-grid">@foreach($relacionadas as $related)<x-recipe-card :recipe="$related" />@endforeach</div></section>
    </article>
</x-layouts.public>
