...
<x-layouts.public :title="$receita->titulo.' — Cantinho das Receitas'" :description="\Illuminate\Support\Str::limit($receita->descricao, 155)" :image="$receita->foto_url ?: null" og-type="article">
    ...
    <article class="recipe-detail container">
        ...
        <div class="detail-hero"><img src="{{ $receita->foto_url ?: 'https://placehold.co/1200x700/f3eadf/bd5b32?text=Receita' }}" alt="{{ $receita->titulo }}"><div class="detail-hero-overlay"><span class="recipe-category">{{ $receita->categoria->nome }}</span><h1>{{ $receita->titulo }}</h1><p>{{ $receita->descricao }}</p></div></div>
