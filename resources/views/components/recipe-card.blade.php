...
<article class="recipe-card">
    <a href="{{ isset($card['slug']) ? route('receitas.mostrar', $card['slug']) : route('receitas.listar', ['busca' => $card['title']]) }}" class="recipe-image-wrap">
        ...
        <span class="recipe-save" aria-label="Salvar receita">♡</span>
    </a>
    <div class="recipe-card-body">
        <h3><a href="{{ isset($card['slug']) ? route('receitas.mostrar', $card['slug']) : route('receitas.listar', ['busca' => $card['title']]) }}">{{ $card['title'] }}</a></h3>
        <div class="recipe-meta"><span>◷ {{ $card['time'] }}</span><span>•</span><span>{{ $card['difficulty'] }}</span></div>
        <div class="recipe-rating"><span>★</span><strong>{{ $card['rating'] }}</strong><small>({{ $card['reviews'] }})</small></div>
    </div>
</article>
